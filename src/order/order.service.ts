...
export class OrderService {
  ...
  async createOrder(createOrderDto: CreateOrderDto, user: ActiveUserInterface): Promise<Order> {
  ...
  async removeProductFromOrder(orderId: string, productId: string): Promise<void> {
    const orderItem = await this.orderItemService.findOrderItemByOrderIdAndProductId(orderId, productId);
    
    if(!orderItem) {
      throw new NotFoundException(`Order item with product id ${productId} not found for order id ${orderId}`);
    }
